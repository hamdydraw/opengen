...
<script>
    export default {
        title () {  return 'Add language - '+this.$appName;},
         data () {
          return {
          editMode:false,  
           ...
           editRecords()
             {
               if(this.$route.params.id>0)
               {
                 ...
                 axios.get(this.$baseUrl+"api/getLanguage/"+this.$route.params.id).then(
                   ({data})=>
                    {
                    this.form.fill(data); 
             ...
             {
                this.$Progress.start() 
                this.form.post(this.$baseUrl+'api/language')
                .then(()=>{
             ...
             {
                this.$Progress.start() 
                this.form.put(this.$baseUrl+'api/language/'+this.$route.params.id)
                .then(()=>{
                  Fire.$emit('AfterCreate'); 
                  Toast.fire({
                      type: 'success',
