          ...
          <div class="col-12">
            <div class="card">
              ...
              <div class="card-header">
                <h3 class="card-title">Add new length class</h3> 
...
<script>
    export default {
        title () {  return 'Add length class - '+this.$appName;},
         data () {
          return {
          editMode:false,  
           ...
           editRecords()
             {
               if(this.$route.params.id>0)
               {
                 ...
                 axios.get(this.$baseUrl+"api/getLengthclass/"+this.$route.params.id).then(
                   ({data})=>
                    {
                    this.form.fill(data);
                    this.form.value=data.lengthclass.value;
             ...
             loadRecords()
             {
                ...
                this.$Progress.start(); 
                 axios.get(this.$baseUrl+"api/lengthclasslookups").then(
                   ({data})=>
                    {
             ...
             {
                this.$Progress.start() 
                this.form.post(this.$baseUrl+'api/lengthclass')
                .then(()=>{
             ...
             {
                this.$Progress.start() 
                this.form.put(this.$baseUrl+'api/lengthclass/'+this.$route.params.id)
                .then(()=>{
                  Fire.$emit('AfterCreate'); 
                  Toast.fire({
                      type: 'success',
                      title: 'lengthclass updated successfully'
                    });
                 this.$Progress.finish();
                })
                .catch(()=>{
